...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const form = document.getElementById('DataAssetForm'); // เปลี่ยนเป็น ID ของฟอร์มคุณ

        form.addEventListener('submit', function(e) {
            e.preventDefault(); // ป้องกันการส่งฟอร์ม

            const vehicleChassis = document.getElementById('Vehicle_Chassis').value; // เปลี่ยนเป็น ID ของฟิลด์ chassis

            // เช็คว่า vehicleChassis เป็นค่าว่างหรือไม่
            if (!vehicleChassis) {
                Swal.fire({
                    icon: 'warning',
                    title: 'กรุณากรอกเลขตัวถัง!',
                    text: 'เลขตัวถังไม่สามารถเป็นค่าว่างได้'
                });
                return;
            }
            ...
            fetch(form.action, {
                ...
                headers: {
                    ...
                    'X-CSRF-TOKEN': '{{ csrf_token() }}' // อย่าลืมเพิ่ม CSRF token
                },
                body: JSON.stringify({
                    Vehicle_Chassis: vehicleChassis,
                    // เพิ่มฟิลด์อื่นๆ ที่ต้องการส่งไปยังเซิร์ฟเวอร์
                })
            })
            .then(response => {
                if (response.ok) {
                    Swal.fire({
                        ...
                        text: 'ข้อมูลสินทรัพย์ของคุณถูกสร้างเรียบร้อยแล้ว'
                    });
                    // รีเฟรชหน้าหรือทำการอัปเดต UI ตามที่ต้องการ
                } else if (response.status === 409) {
                    return response.json().then(data => {
                        Swal.fire({
                            icon: 'error',
                            title: 'เกิดข้อผิดพลาด!',
                            text: data.message_error_chassis
                        });
                    });
                } else {
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'เกิดข้อผิดพลาด!',
                    text: 'ไม่สามารถสร้างข้อมูลได้ กรุณาลองใหม่อีกครั้ง'
                });
            });
        });
    });
...
{{-- <script>
    // ตัวอย่างฟังก์ชัน Ajax ที่เรียกใช้ store
    function storeAsset() {
        $.ajax({
            url: '/data_assets', // เปลี่ยนเป็น URL ที่คุณใช้เรียกใช้งาน
            type: 'POST',
            data: $('#DataAssetForm').serialize(), // เปลี่ยนเป็น ID ของฟอร์มที่คุณใช้
            success: function(response) {
                // แสดง SweetAlert เมื่อสร้างข้อมูลสำเร็จ
                Swal.fire({
                    title: 'สำเร็จ!',
                    text: response.message + '\nเลขตัวถัง: ' + response.Vehicle_Chassis,
                    icon: 'success',
            ...
            error: function(xhr) {
                if (xhr.status === 409) {
                    // แสดง SweetAlert หากมีเลขตัวถังซ้ำ
                    Swal.fire({
                        title: 'ผิดพลาด!',
